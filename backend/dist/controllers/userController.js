    ...
    static changePassword(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                // 获取用户信息
                const user = yield User_1.UserModel.findByUsername((_b = req.user) === null || _b === void 0 ? void 0 : _b.username);
                if (!user) {
                    return res.status(404).json({
                        success: false,
                        message: '用户不存在'
                ...
                // 更新密码
                const success = yield User_1.UserModel.update(userId, { password: hashedNewPassword });
                if (!success) {
                    return res.status(500).json({
                        success: false,
                        message: '密码更新失败'
